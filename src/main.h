...
public:
    ...
    {
        // Pulse: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Pulse: get transaction coin age
...
public:
    ...
    // Pulse: block signature - signed by coin base txout[0]'s owner
    ...
    // Pulse: two types of block: proof-of-work or proof-of-stake
    ...
    // Pulse: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Pulse: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Pulse: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Pulse: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Pulse: block index flags
    enum  
    {
